...
| Phase | A discrete stage in the RDD pipeline (RESEARCH, PRODUCT DISCOVERY, MODEL, DECIDE, ARCHITECT, BUILD, and optionally SYNTHESIS). Each phase produces an artifact and ends at a gate — except SYNTHESIS, which is a terminal phase with a different structure. | Gate, Artifact, Pipeline, Synthesis |
| Pipeline | The ordered sequence of RDD phases from RESEARCH through BUILD, with an optional SYNTHESIS phase after the terminal phase. Epistemic acts at each gate compound across the pipeline. The synthesis essay closes the epistemic loop opened by RESEARCH. | Phase, Synthesis |
...
| Synthesis | An optional terminal RDD phase that extracts publishable insight from the artifact trail. The agent mines the trail for novelty signals and conducts a structured conversation (journey review, novelty surfacing, framing) to help the user find the story worth telling. The deliverable is a co-produced outline; the user writes the essay. Not a gate — has a different cost structure (conversation bounded at 30-60 minutes; writing is the user's own work, on their own time). Available after BUILD or after whatever phase terminates the current pipeline run. | Phase, Synthesis Essay, Artifact Trail, Novelty Signal, Dead Discovery |
| Synthesis Essay | The user-written publishable essay produced from the synthesis outline. A dual-purpose artifact: publishable narrative for the world and narrative context rollup for the pipeline. Written in the user's own voice — the agent does not write it. Captures *understanding* (the story of why things are the way they are) where domain models capture vocabulary, system designs capture architecture, and ADRs capture decisions. The final epistemic act of the RDD cycle. | Synthesis, Narrative Context Rollup, Public Authority, Writing as Inquiry |
...
| Artifact Hierarchy | The three-tier structure of the RDD artifact corpus. Tier 1: orientation document (entry point — routes readers). Tier 2: primary readable documents (product-discovery.md for product stakeholders, system-design.md for technical stakeholders — read end-to-end). Tier 3: supporting material (domain model, essays, ADRs, scenarios — consulted as needed for provenance and depth). Replaces the prior two-tier model (primary readables + supporting material) by adding the orientation tier above. | Orientation Document, Artifact, Artifact Legibility |
...
| Frame Narrative | AI + User | Synthesis Essay | Co-produce the essay's central question, volta, and structural form. The agent offers narrative frameworks as lenses to try on (ABT sentence, story spine, braided structure, etc.) — not templates to fill in. The writer may adopt, modify, or discard any of these in favor of their own structural impulse. The outline format should get the writer going, not constrain them. Output is an outline that opens doors rather than prescribing a path |
...
- Synthesis **follows** terminal Phase (BUILD, or ARCHITECT in scoping mode)
